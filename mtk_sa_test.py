...
def send_recv_data_thd(SERIAL, item_idx, data):
    ...
    if (SERIAL.port_is_open()):
        ...
        # send log
        log_str="send>>>" + binascii.b2a_hex(bytearray(data)).decode('utf-8')
        LOG.info(log_str)
        ...
        # recv log
        log_str="recv<<<" + binascii.b2a_hex(bytearray(SERIAL.message)).decode('utf-8')
        LOG.info(log_str)
        listbox.insert(END,log_str)
...
def init_form_by_config():
    ...
    for section  in sec:
        ...
        if  section.find('sa_item') == 0:   #pos = 0
            ...
            btn_name['btn%s'%item_idx] = Button(root, text='测 试', width=25, command=fn_obj)
